...
class Request extends \ActionRequest {
	...
	public function handle_live_mode( $order ) {
		...
		$signingKey = $this->GetPrivateKey( $this->gateway->get_option( 'private_signing_key' ) );
		// Merchant Private Signing Key.
		...
		$response = wp_remote_post( 'https://core.paco.2c2p.com/api/1.0/Payment/prePaymentUi', $options );

		\WC_Gateway_HBL_Payment::log( 'Response details for ' . $order->get_order_number() . ': ' . wc_print_r( $response, true ) );

		$token = wp_remote_retrieve_body( $response );

		$decryptingKey = $this->GetPrivateKey( $this->gateway->get_option( 'private_encryption_key' ) );
		// Merchant Private Encryption Key.
	...
	protected function get_hbl_payment_args( $order ) {
		...
		$product_details = $this->build_product_description( $order );
		...
		return array(
			...
			'notificationURLs'   => array(
				...
				'failedURL'       => $this->notify_url . '?failed=true',
				'cancellationURL' => $this->notify_url . '?canceled=true',
	...
	/**
	 * Build product details to pass to HBL.
	 *
	 * @param  WC_Order $order Order object.
	 *
	 * @since 1.0.0
	 ...
	 */
	protected function build_product_description( $order ) {
		...
		foreach ( $items as $item ) {
			...
			$products .= $item_data['name'] . ' - ' . $item_data['quantity'];
